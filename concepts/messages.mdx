...
<AccordionGroup>
  ...
  <Accordion title="工具调用响应结构" icon="info-circle">
    **说明**：启用工具调用时，API 响应包含 `dynamic-tool` parts

    ```json 工具调用响应示例 lines expandable icon="check"
    {
      "messages": [
        {
          "id": "msg_001",
    ...
    <Tip>
      工具调用和结果包含在同一个 `dynamic-tool` part 中。服务端自动执行工具并管理 `toolCallId`，客户端无需手动处理。详见[工具调用](/features/tool-calling)
    </Tip>
